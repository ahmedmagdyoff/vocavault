      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex h-16 items-center border-b border-slate-200 px-6 dark:border-slate-800">
            <h1 className="text-xl font-bold text-blue-600 dark:text-blue-500">VocaVault</h1>
          </div>
          ...
          <nav className="flex-1 space-y-1 px-4 py-6">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`mr-3 h-5 w-5 flex-shrink-0 ${isActive ? 'text-blue-700 dark:text-blue-400' : 'text-slate-400'}`} />
                  {item.name}
                </Link>
              );
            })}
